...
class GioHangController extends Controller
{
    ...
    public function del(Request $request)
    {
        ...
        if ($data) {
            ...
            $data->delete();
            $updateSoLuong =  $xe->so_luong + $request->so_luong;
            $xe->update([
    ...
    public function update(Request $request)
    {
        ...
        if ($gioHang) {
            if ($request->so_luong > 100) {
                $suaSL = $gioHang->so_luong;
                $xe->update([
                    'so_luong' => $xe->so_luong - 100 + $suaSL,
                ]);
                $gioHang->so_luong = 100;
                $gioHang->save();
                return response()->json([
                    'status'    => -2,
                    'soLuong' => $xe->so_luong - 100 + $suaSL,
                    'message'   => 'Số lượng > 100, giá thuê/sỉ sẽ không được áp dụng, liên hệ trực tiếp chúng tôi để biết thêm thông tin',
                ]);
            } else if ($request->so_luong < 1) {
                $suaSL = $gioHang->so_luong;
                $xe->update([
                    'so_luong' => $xe->so_luong - 1 + $suaSL,
                ]);
                $gioHang->so_luong = 1;
                $gioHang->save();
                return response()->json([
                    'status'    => -1,
                    'soLuong' => $xe->so_luong - 1 + $suaSL,
